# checkpoint_nchunks=0 like torch2
# Try releasing after batch
...
device =  "cuda:7"
epochs = 5
best_model_path = "/u/amagzari/wattrel-amagzari/REPOS/FGC/models/model_torch3"
...
class ViTEncoder(nn.Module):
    ...
    def __init__(self, vit, nblocks=12, checkpoint_nchunks=0, return_attn_wgts=True):
...
class MultiCropViT(nn.Module):
    ...
    def __init__(self, 
                 ...
                 encoder_nblocks=12, checkpoint_nchunks=0):
    ...
    def forward(self, xb_high_res):
        ...
        # Use no_grad here since this is just for attention map, not for backward
        #with torch.no_grad():
        ...
        # ⚠️ Must re-interpolate to disconnect from earlier graph
        #xb_input_res = nn.functional.interpolate(xb_high_res, size=(self.input_res,self.input_res))
...
_encoder = timm.create_model(arch, pretrained=True, in_chans=3)
...
mcvit_model = MultiCropViT(
    ...
    encoder_nblocks=12, checkpoint_nchunks=0, **model_config
).to(device)

optimizer = torch.optim.AdamW(mcvit_model.parameters(), lr=3e-4)
scaler = GradScaler()
best_val_acc = 0.0
